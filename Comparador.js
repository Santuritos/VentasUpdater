...
function colorearPreciosVentas() {
  ...
  if (!hojaVentas) {
    ...
    throw new Error("La hoja 'Ventas' no existe.");
  }
  ...
  if (colIndexHamelyn === -1 || colIndexMomox === -1 || colIndexAmmareal === -1 || colIndexVenta === -1) {
    ...
    throw new Error("No se encontraron las columnas 'Hamelyn', 'Momox', 'Ammareal' o 'Venta' en la hoja Ventas.");
  }
  ...
  for (let i = 1; i < datos.length; i++) {
    ...
    if (valorVenta === "enviado") {
      Logger.log(`Fila ${i + 1}: Aplicando color rojo claro (${colorVentaEnviado}) por 'Venta' = 'Enviado'`);
